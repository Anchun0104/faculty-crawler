...
- 默认桌面入口是原生 PySide6 界面，左侧导航可折叠；仅在诊断兼容场景设置 `FACULTY_CRAWLER_LEGACY_UI=1` 才会启动保留的 Tk 界面。
...
在桌面端的“设置 / AI 设置”中添加、替换或删除 API Key。密钥使用当前 Windows 用户的 DPAPI 保护，界面仅显示“已配置”状态，不会显示、导出或写入日志明文。AI 用量页按本月汇总调用次数、输入/输出 Token 和预估费用；未配置密钥时仍可完成本地采集。

### 一次粘贴多个 URL

点击“新建采集”，可在多行输入框一次粘贴任意数量的教师目录 URL（每行一个；为便于检查，建议每批约 20 个）。程序会先检查格式、重复项和访问合规提示；确认后创建 **1 个批次任务，包含 N 所学校**，并按顺序执行。为保留同一批次的恢复、运行记录和统一导出语义，它不会把这些 URL 伪装成 N 个相互独立的数据库任务。
