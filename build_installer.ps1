...
if ($Version -notmatch '^\d+\.\d+\.\d+$TempRoot = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
...
try {
    ...
    & $InnoCompiler "/O$InstallerRoot" "/DAppVersion=$Version" "installer\faculty-crawler.iss"
    ...
    $InstallerHash = (Get-FileHash -LiteralPath $InstallerExe -Algorithm SHA256).Hash
