...
class ReleasePackageTests(unittest.TestCase):
    ...
    def test_installer_build_propagates_version_to_all_artifacts(self) -> None:
        ...
        self.assertIn(r"'^\d+\.\d+\.\d+    def test_installer_build_sources_exist_and_use_explicit_inputs(self) -> None:
        paths = (
    ...
    def test_build_script_is_clean_fail_fast_and_validates_artifacts(self) -> None:
        ...
        self.assertIn("FacultyCrawler-Setup.exe", text)
...
if __name__ == "__main__":
    unittest.main()
", build_text)
        self.assertIn("$WindowsFileVersion = \"$Version.0\"", build_text)
        self.assertIn("FACULTY_CRAWLER_VERSION_FILE", build_text)
        self.assertIn('"/DAppVersion=$Version"', build_text)
        self.assertIn('"FacultyCrawler-Setup-$Version.exe"', build_text)
        self.assertIn("Get-FileHash", build_text)
        self.assertIn("git rev-parse --short HEAD", build_text)
        self.assertIn("[Convert]::FromBase64String", build_text)
        self.assertNotIn("高校教师信息采集工具", build_text)
        self.assertNotIn("高校教师目录批量采集与 Excel 导出工具", build_text)

        self.assertIn("FACULTY_CRAWLER_VERSION_FILE", spec_text)
        self.assertIn("version=str(version_file)", spec_text)

        self.assertNotIn('#define AppVersion "1.0.0"', inno_text)
        self.assertIn("AppVersion={#AppVersion}", inno_text)
        self.assertIn("OutputBaseFilename=FacultyCrawler-Setup-{#AppVersion}", inno_text)
